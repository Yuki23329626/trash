...
BigInt::BigInt(short int* num, short int tmpLength, bool tmpSign)
    : digit(), length(), sign()
{
    sign = tmpSign;
...
short int BigInt::getLength() const
{
    return length;
}
...
{
    short int *digit1 = amount1.digit, *digit2 = amount2.digit;
...
const BigInt operator *(const BigInt& amount1, const BigInt& amount2)
{
    ...
    short int i,j,carry;
    short int *sum = new short int[amount1.length + amount2.length];
    short int *digit1 = amount1.digit, *digit2 = amount2.digit;
    ...
    if(amount1.sign == amount2.sign){
        BigInt result(sum, amount1.length + amount2.length, 0);
        delete [] sum;
        return result;
    }
    else{
        BigInt result(sum, amount1.length + amount2.length, 1);
        delete [] sum;
        return result;
    }
}
...
const BigInt BigInt::leftShift(short int times) const // this *= 10^times
{
    short int i, *arr = new short int[length + times];
    ...
    BigInt result(arr, length + times, sign);
    delete [] arr;
    return result;
}
...
const BigInt operator /(const BigInt& amount1, const BigInt& amount2)
{
    ...
    quotient.digit = new short int[quotient.length];
    for(short int i = 0 ; i < quotient.length ; i++)
        quotient.digit[i] = 0;
...
const BigInt operator +(const BigInt& amount1, const BigInt& amount2)
{
    ...
    if(amount1.sign == amount2.sign)
    {
        if(amount1.length >= amount2.length)
        {
            ...
            sum = new short int[amount1.length+1];
            ...
            BigInt result(sum, amount1.length+1, amount1.sign);
            delete [] sum;
            return result;
        }
...
const BigInt operator -(const BigInt& amount1, const BigInt& amount2)
{
    ...
    if(amount1.sign == amount2.sign)
    {
        if(amount1.length > amount2.length)
        {
            ...
            BigInt result(&sum[len+1], amount1.length - len, amount1.sign);
            delete [] sum;
            return result;
        }
        else if(amount1.length == amount2.length)
...
BigInt& BigInt::operator =(const BigInt& rightObj)
{
    ...
    {
        delete [] digit;
        digit = new short int[rightObj.length];
    }
