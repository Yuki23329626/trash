...
public:
    ...
    BigInt(int num);
    BigInt(short int* num, short int tmpLength, bool tmpSign);
    BigInt();
    BigInt(const BigInt& iniObj);

    // accessor function
    const BigInt abso() const;  // get absolute value of the bigInt object
    short int getLength() const;  // get length of the bigInt object (without the sign)
    bool getSign() const;  // get the sign of the bigInt object
    const BigInt rightShift() const;
    const BigInt leftShift(short int times) const;
